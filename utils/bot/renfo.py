    ...
    for i, (bgm_id, tmdb_d, up_folder_name) in enumerate(todo_list):
        ...
        if tmdb_d:
            try:
                ...
                return await bot.reply_to(message, f"[{i+1}/{todo_num}] 获取生成 Season NFO 数据失败:\n\n{e}")
        ...
        try:
            ...
            return await bot.reply_to(message, f"[{i+1}/{todo_num}] 获取文件列表失败: 未找到文件夹 {up_folder_name}")
        if dirs_list:
            ...
            return await bot.reply_to(message, f"[{i+1}/{todo_num}] 获取文件列表失败: 未找到 {up_folder_name} 文件夹下的视频文件")
        try:
            proc = await asyncio.create_subprocess_exec(
            ...
            await bot.reply_to(message, f"[{i+1}/{todo_num}] 上传失败: {e}")
        finally:
